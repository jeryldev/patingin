...
use crate::core::{Language, Severity, ProjectDetector, CustomRulesManager, CustomRule};
...
fn determine_target_languages(args: &RulesArgs) -> Result<Vec<Language>> {
    ...
    if args.elixir { languages.push(Language::Elixir); }
    if args.javascript { languages.push(Language::JavaScript); }
    if args.typescript { languages.push(Language::TypeScript); }
    if args.python { languages.push(Language::Python); }
    if args.rust { languages.push(Language::Rust); }
    if args.zig { languages.push(Language::Zig); }
    if args.sql { languages.push(Language::Sql); }
...
fn show_rule_detail(registry: &crate::core::registry::PatternRegistry, rule_id: &str) -> Result<()> {
    use colored::*;
    ...
    if let Some(rule) = registry.get_pattern(rule_id) {
        ...
        println!("Severity: {}", match rule.severity {
        ...
        println!("Claude Code Fixable: {}", if rule.claude_code_fixable { "Yes".green() } else { "No".red() });
...
fn handle_add_rule(args: &RulesArgs) -> Result<()> {
    ...
    let pattern = format!(r"{}", description.split_whitespace().last().unwrap_or("TODO"));
...
fn get_language_from_args(args: &RulesArgs) -> Result<Language> {
    match (args.elixir, args.javascript, args.typescript, args.python, args.rust, args.zig, args.sql) {
...
fn handle_remove_rule(rule_id: &str) -> Result<()> {
    ...
    if removed {
        ...
        println!("❌ Rule '{}' not found in project '{}'", rule_id, project_name);
...
fn show_custom_rules(registry: &PatternRegistry, project_name: &str, target_languages: &[Language]) -> Result<()> {
...
fn show_organized_rules(rules: &[&crate::core::AntiPattern], target_languages: &[Language], args: &RulesArgs) -> Result<()> {
    use colored::*;
    ...
    if !args.global && !args.project && !args.all_projects && args.search.is_none() && 
       !args.elixir && !args.javascript && !args.typescript && !args.python && 
       !args.rust && !args.zig && !args.sql {
    ...
    for language in target_languages {
        if let Some(lang_rules) = rules_by_language.get(language) {
            ...
            let (emoji, name) = get_language_display_info(language);
            let critical_count = lang_rules.iter().filter(|p| p.severity == Severity::Critical).count();
            let major_count = lang_rules.iter().filter(|p| p.severity == Severity::Major).count();
            let warning_count = lang_rules.iter().filter(|p| p.severity == Severity::Warning).count();
    ...
    println!("Total: {} rules across {} languages", total_rules, total_languages);
    ...
    println!("💡 Use {} to see detailed info about a specific rule", "patingin rules --detail <rule_id>".cyan());
