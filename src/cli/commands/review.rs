...
use crate::core::{Language, Severity, ReviewEngine, ProjectDetector};
use crate::git::{DiffScope, GitDiffParser};
use crate::external::fix_engine::{FixEngine, BatchFixRequest};
...
pub async fn run(args: ReviewArgs) -> Result<()> {
    ...
    let filtered_violations = if let Some(min_severity) = args.severity {
        review_engine.filter_violations_by_severity(&review_result.violations, min_severity)
            .into_iter()
            .cloned()
            .collect()
    } else {
...
fn filter_diff_by_language(git_diff: crate::git::GitDiff, target_language: &Language) -> crate::git::GitDiff {
    let review_engine = ReviewEngine::new();
    
    let filtered_files = git_diff.files.into_iter()
        .filter(|file_diff| {
...
fn output_json_results(review_result: &crate::core::review_engine::ReviewResult, violations: &[crate::core::ReviewViolation]) -> Result<()> {
    use serde_json;
    ...
    let json_violations: Vec<JsonViolation> = violations.iter().map(|v| JsonViolation {
...
fn show_fix_suggestions(violations: &[crate::core::ReviewViolation]) {
    let auto_fixable: Vec<_> = violations.iter()
...
async fn handle_auto_fix(violations: &[crate::core::ReviewViolation], no_confirm: bool) -> Result<()> {
    let auto_fixable: Vec<_> = violations.iter()
        .filter(|v| v.auto_fixable)
        .cloned()
        .collect();
...
async fn handle_interactive_fix(violations: &[crate::core::ReviewViolation]) -> Result<()> {
    ...
    println!("🔍 Found {} violation(s). Launching interactive Claude Code session...", violations.len());
...
fn create_claude_query(violations: &[crate::core::ReviewViolation]) -> Result<String> {
    ...
    // Group violations by file
    let mut files_with_violations: HashMap<String, Vec<&crate::core::ReviewViolation>> = HashMap::new();
    for violation in violations {
    ...
    // Format languages
    let languages: Vec<String> = project_info.languages
        .iter()
        .map(|l| format!("{:?}", l))
        .collect();
...
mod review_command_tests {
    use super::*;
    use crate::core::{AntiPattern, ReviewViolation, DetectionMethod, Severity, Language};
    ...
    fn test_filter_diff_by_language() {
        use crate::git::{GitDiff, FileDiff, ChangedLine, ChangeType};
