...
use patingin::core::{Language, CustomRulesManager, CustomRule, ReviewEngine};
...
async fn test_multi_language_project_detection() -> Result<()> {
    ...
    assert!(project_root.join("test_files").join("elixir_violations.ex").exists(), "Should have Elixir test files");
    assert!(project_root.join("test_files").join("javascript_violations.js").exists(), "Should have JavaScript test files");
