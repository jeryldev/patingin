...
use patingin::core::{CustomRulesManager, CustomRule, Language, ProjectDetector};
use patingin::cli::commands::{setup, review};
...
"#).expect("Should write invalid YAML");
    ...
    let result = std::panic::catch_unwind(|| {
        custom_rules_manager.get_project_rules("test-project")
    });
...
fn test_malformed_regex_patterns_in_custom_rules() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(error_msg.contains("regex") || error_msg.contains("pattern"), 
                "Error message should mention regex/pattern issue: {}", error_msg);
        }
    }
}
...
fn test_non_existent_project_paths() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            let error_msg = e.to_string();
            assert!(error_msg.contains("path") || error_msg.contains("directory") || error_msg.contains("exist"),
                "Error should mention path/directory issue: {}", error_msg);
        }
    }
}
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                ...
                "Error should be informative about empty repository: {}", error_msg
            );
        }
    }
...
fn test_custom_rules_file_permissions() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            let error_msg = e.to_string();
            assert!(error_msg.contains("permission") || error_msg.contains("access") || error_msg.contains("file"),
                "Error should be about file access: {}", error_msg);
        }
    }
}
...
fn test_large_configuration_file_handling() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            let error_msg = e.to_string();
            assert!(!error_msg.contains("too large") && !error_msg.contains("size"),
                "Should not fail due to file size: {}", error_msg);
        }
    }
}
