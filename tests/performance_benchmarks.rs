...
fn test_rule_registry_scalability() {
    ...
    assert!(
        ...
        "Adding 5 rules should complete within 2s, took {}ms",
        duration.as_millis()
    );
    ...
    assert!(
        retrieval_duration.as_millis() < 150,
        "Rule retrieval should complete within 150ms, took {}ms",
    ...
    println!(
        "✅ Rule scalability test: 5 rules added in {}ms, retrieved in {}ms",
        duration.as_millis(),
        retrieval_duration.as_millis()
    );
}
...
fn test_pattern_matching_performance() {
    ...
    assert!(
        registry_load_time.as_millis() < 200,
    ...
    assert!(
        matching_duration.as_millis() < 500,
        "Pattern matching on large content should complete within 500ms, took {}ms",
        matching_duration.as_millis()
    );

    println!(
        "✅ Pattern matching test: Registry loaded in {}ms, matching completed in {}ms",
        registry_load_time.as_millis(),
        matching_duration.as_millis()
    );
}
...
fn test_startup_time_measurement() {
    ...
    assert!(
        total_time.as_millis() < STARTUP_TIME_LIMIT_MS,
    ...
    println!(
        "✅ Startup time test: Registry={}ms, Engine={}ms, Detector={}ms, Rules={}ms, Total={}ms",
        registry_time.as_millis(),
        engine_time.as_millis(),
        detector_time.as_millis(),
        rules_time.as_millis(),
...
fn test_concurrent_review_performance() {
    ...
    assert!(
        ...
        "Concurrent reviews should complete within 3s, took {}ms",
        duration.as_millis()
    );
